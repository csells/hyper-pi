...
export default function piSocket(pi) {
    const nodeId = `${os.hostname()}-${process.pid}`;
    let wss = null;
    let hypivisorWs = null;
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        wss.on("connection", boundary("wss.connection", (ws) => {
            ...
            const initPayload = buildInitState(ctx.sessionManager.getBranch(), pi.getAllTools());
    ...
    function connectToHypivisor(port) {
        if (!hypivisorUrlValid)
            return;
        const url = hypiToken
